 ...
 *
 * Sizes are auto-detected by scripts/auto_montserrat.py at build time —
 * any `lv_font_montserrat_NN` referenced in src/ (including SLS-exported
 * src/UI/*.c) gets a `-DLV_FONT_MONTSERRAT_NN=1` flag automatically.
 *
 * The defaults below are wrapped in #ifndef so the build-flag from the
 * auto-detect script wins. Override here only if you want a size enabled
 * even when no code references it. */
...
#define LV_FONT_MONTSERRAT_14 1   /* LV_FONT_DEFAULT — keep regardless */
#endif
...
#define LV_USE_SYSMON   1   /* needed for runtime FPS overlay at DEBUG_RENDERING_PERF */
#if LV_USE_SYSMON
    ...
    #define LV_USE_PERF_MONITOR 1   /* compiled in; shown only at DEBUG_RENDERING_PERF */
